...
async function handleLoginVerify(event) {
  ...
  try { const result = await apiPost({ action: "verifyLoginCode", email: pendingLoginEmail, code }); auth = { email: result.email, token: result.token, loginAt: new Date().toISOString() }; saveAuth(); resetAuthForms(); renderAuthState(); await syncFromServer(); notify("נכנסת בהצלחה", "success"); }
...
async function loginWithBiometrics(automatic = false) {
  ...
  try {
    ...
    auth = { email: unlocked.email, token: unlocked.token, loginAt: new Date().toISOString() };
    saveAuth();
    renderAuthState();
