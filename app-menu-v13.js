...
const els = {
  ...
  settingsForm: document.querySelector("#settingsForm"), summaryEmail: document.querySelector("#summaryEmail"), sheetsUrl: document.querySelector("#sheetsUrl"), exportAccountDataBtn: document.querySelector("#exportAccountDataBtn"), deleteAccountBtn: document.querySelector("#deleteAccountBtn"), installBtn: document.querySelector("#installBtn"), refreshBtn: document.querySelector("#refreshBtn"),
...
function saveAuth() { if (auth) { const biometric = loadBiometric(); const stored = biometric?.email === auth.email ? { email: auth.email, biometricLocked: true } : auth; localStorage.setItem(AUTH_KEY, JSON.stringify(stored)); } else localStorage.removeItem(AUTH_KEY); }
...
async function loginWithBiometrics(automatic = false) {
  ...
  try {
    ...
    auth = { email: unlocked.email, token: unlocked.token };
    saveAuth();
    renderAuthState();
